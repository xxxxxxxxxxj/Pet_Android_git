 ...
 */
@SuppressLint("NewApi")
public class TopIndicator_T extends LinearLayout {
	...
	// 顶部菜单的文字数组
	private CharSequence[] mLabels = new CharSequence[] { "全　部", "进行中", "待评价",
			"已取消" };
	...
	public TopIndicator_T(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public TopIndicator_T(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public TopIndicator_T(Context context) {
		super(context);
		init(context);
	}
	...
	private void init(final Context context) {
		...
		mUnderLine.setBackgroundColor(context.getResources().getColor(
				R.color.aD1494F));
		...
		for (int i = 0; i < size; i++) {
			...
			final View view = inflater.inflate(R.layout.top_indicator_item_t,
					null);

			final TextView tvName = (TextView) view
					.findViewById(R.id.tv_order_name);
			final TextView tvFlag = (TextView) view
					.findViewById(R.id.tv_order_flag);
	...
	public void setTabsDisplay(Context context, int index) {
		...
		for (int i = 0; i < size; i++) {
			...
			if ((Integer) (kttab.getTag()) == index) {
				kttab.setTextColor(context.getResources().getColor(
						R.color.aD1494F));
			} else {
	...
	public void setFlagDisplay(int num) {
		if (mFlagList.size() > 1 && num > 0) {
			...
			TextView ktflag = mFlagList.get(2);
			ktflag.setVisibility(View.GONE);
			if ((Integer) (ktflag.getTag()) == 2) {
				ktflag.setText("" + num);
