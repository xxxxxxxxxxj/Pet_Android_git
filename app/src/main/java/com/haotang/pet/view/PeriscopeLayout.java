...
import com.haotang.pet.util.BezierEvaluator;
...
public class PeriscopeLayout extends RelativeLayout {
	...
	private void init() {
		...
		Drawable red = getResources().getDrawable(R.drawable.img_flower);
		Drawable yellow = getResources().getDrawable(R.drawable.img_shit);
	...
	public void addHeart(int flag) {

		ImageView imageView = new ImageView(getContext());
		// 随机选一个
		imageView.setImageDrawable(drawables[flag]);
	...
	private PointF getPointF(int scale) {
		...
		pointF.x = random.nextInt((mWidth - 100) > 0 ? (mWidth - 100) : mWidth);// 减去100
																				// 是为了控制
																				// x轴活动范围,看效果
																				// 随意~~
		// 再Y轴上 为了确保第二个点 在第一个点之上,我把Y分成了上下两半 这样动画效果好一些 也可以用其他方法
		pointF.y = random.nextInt((mHeight - 100) > 0 ? (mHeight - 100)
				: mHeight) / scale;
		return pointF;
	}
