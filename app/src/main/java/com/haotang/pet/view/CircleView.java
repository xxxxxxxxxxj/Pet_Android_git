...
public class CircleView extends View {
	...
	public CircleView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		TypedArray array = context.getTheme().obtainStyledAttributes(attrs,
				R.styleable.myCircleImage, defStyleAttr, 0);
		@SuppressWarnings("unused")
		int leng = array.length();
		// 获取自定义的属性，这里注意是R.styleable.myCircleImage_circlecolor而不是R.attr.circlecolor
		circleBack = array.getColor(R.styleable.myCircleImage_circlecolor,
				Color.GREEN);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mschedual <= 100) {// ，如果当前进度小于100，画实心矩形
			...
			canvas.drawRect(width / 2 - circleHalf, height / 2 - circleHalf,
					width / 2 + circleHalf, height / 2 + circleHalf - mschedual
							* circleHalf / 50, paint);
		}
		// 画当前进度的字符串
		...
		percent = mschedual + " %";
		canvas.drawText(percent, width / 2 - getTextWidth(percent, 30) / 2,
				height / 2 + paint.getTextSize() * 3 / 8, paint);// 字体的高度=paint.getTextSize()*3/4
		// 画空心圆
		paint.setColor(circleBack);
