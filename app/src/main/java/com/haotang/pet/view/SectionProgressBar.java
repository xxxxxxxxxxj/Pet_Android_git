 ...
 * 
 * @author 徐俊
 * @date 2016-12-21 下午3:12:16
 */
public class SectionProgressBar extends View {
	private static final int DEFAULT_BACKGROUND_COLOR = Color
			.parseColor("#ececec");
	private static final int DEFAULT_FOREGROUND_COLOR = Color
			.parseColor("#b93a2c");
	...
	private static final int DEFAULT_LIGHT_TEXT_COLOR = Color
			.parseColor("#89000000");
	private static final float DEFAULT_INITIAL_RATIO = 0.05f;// just pinned zero
																// position
	...
	public SectionProgressBar(Context context, AttributeSet attrs,
		...
		mLevelValues = context.getResources().getIntArray(
				R.array.SectionLevelValues);
		TypedArray ta = context.obtainStyledAttributes(attrs,
				R.styleable.SectionProgressBar, defStyleAttr, 0);
		mBackgroundColor = ta.getColor(
				R.styleable.SectionProgressBar_section_background,
				DEFAULT_BACKGROUND_COLOR);
		mForegroundColor = ta.getColor(
				R.styleable.SectionProgressBar_section_foreground,
				DEFAULT_FOREGROUND_COLOR);
		mSpaceColor = ta.getColor(
				R.styleable.SectionProgressBar_section_space_color,
				DEFAULT_SPACE_COLOR);
		mTextColor = ta.getColor(
				R.styleable.SectionProgressBar_section_text_color,
				DEFAULT_TEXT_COLOR);
		mLightTextColor = ta.getColor(
				R.styleable.SectionProgressBar_section_light_text_color,
				DEFAULT_LIGHT_TEXT_COLOR);
		mTextSize = ta.getDimension(
				R.styleable.SectionProgressBar_section_text_size, sp2px(12));
		mLightTextSize = ta.getDimension(
				R.styleable.SectionProgressBar_section_text_size, sp2px(12));
		mBarHeight = ta.getDimension(
				R.styleable.SectionProgressBar_section_bar_height, dp2px(5));
		mCursorDrawable = ta
				.getDrawable(R.styleable.SectionProgressBar_section_bar_cursor);
		ta.recycle();
		initializePaints();
	}
	...
	private void drawBackground(Canvas canvas) {
		...
		canvas.drawRoundRect(rectF, mBarHeight / 2.f, mBarHeight / 2.f,
				mBackgroundPaint);
	}
	...
	private void drawForeground(Canvas canvas) {
		...
		rectF.left = getPaddingLeft();
		rectF.top = getHeight() / 3.f - mBarHeight / 2.f;
		rectF.right = getWidth() * mRatio - getPaddingRight(); // the dynamic
																// position
		rectF.bottom = getHeight() / 3.f + mBarHeight / 2.f;
		canvas.drawRoundRect(rectF, mBarHeight / 2.f, mBarHeight / 2.f,
				mForegroundPaint);
		...
		cursorRect.left = (int) (rectF.right - mCursorDrawable
				.getIntrinsicWidth() / 2);
		cursorRect.right = (int) (rectF.right + mCursorDrawable
				.getIntrinsicWidth() / 2);
		cursorRect.bottom = (int) (rectF.top - dp2px(5));
		cursorRect.top = cursorRect.bottom
	...
	private void drawSectionAndSubscript(Canvas canvas) {
		...
		float effective_width = getWidth() - getPaddingLeft()
				- getPaddingRight();
		float section_span = (effective_width - effective_width * 0.1f)
				/ (mLevelValues.length - 1);
		// 初始化分段符位置
		RectF rectF = new RectF();
		rectF.left = getPaddingLeft() + getWidth() * 0.05f - section_width
				/ 2.f;
		rectF.top = getHeight() / 3.f - mBarHeight / 2.f;
		...
		for (int i = 0; i < mLevelValues.length; i++) {
			...
			canvas.drawText(String.valueOf(mLevelValues[i]), mLevelX,
					mLevelValueY, mLightTextPaint);
		}
	}
	...
	public void setCurrent(int current) {
		if (current < 0) {
			throw new IllegalArgumentException(
					"current value not allowed for negative numbers");
		}
		...
		mRatioAnimator.setFloatValues(DEFAULT_INITIAL_RATIO,
				mPolicy.computeProgressRatio(current));
		mRatioAnimator
